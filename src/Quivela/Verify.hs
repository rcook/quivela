...
newtype Verify a = Verify { unVerify :: RWST () () VerifyState IO a }
...
-- | Run a Verify action
runVerify :: Verify a -> IO a
runVerify act = do
  initState <- newVerifyState
  (res, state, _) <- runRWST (unVerify act) () initState
  return res
...
methodEquivalenceVCs mtd invs args
  ...
  ctxH1' <- havocContext ctx1'
  results <- liftIO $ symEval' (ECall (EConst (VRef a1)) (mtd ^. methodName) (map EConst  args), ctxH1, pathCond1)
  results' <- liftIO $ symEval' (ECall (EConst (VRef a1')) (mtd ^. methodName) (map EConst  args), ctxH1', pathCond1')
...
freshEmitterVar :: String -> Emitter String
freshEmitterVar prefix' = do
...
translateVar :: String -> Emitter String
translateVar v = do
  translated <- gets (\s -> s ^? (varTranslations . ix v))
  case translated of
    Just tv -> return tv
    Nothing -> do
      tv <- freshEmitterVar v -- TODO: think if we really need something fresh here, since
...
collectSymVars :: Data p => p -> [SymValue]
collectSymVars vc = nubBy (\x y -> symVarName x == symVarName y) $ listify isSymVar vc
...
symValToDafny (SymVar s t) = translateVar s
...
propToDafny (Forall formals p) =
  concatM [ pure "forall "
          , intercalate ", " . map ((++ ": Value")) <$> mapM (translateVar . fst) formals
          , pure " :: "
          , toDafny p ]
...
vcToDafny vc = do
  lemName <- freshEmitterVar $ vc ^. conditionName
  lemArgs <- mapM (translateVar . symVarName) $ collectSymVars vc
...
propToZ3 (Forall formals p) = do
  argNames <- mapM (translateVar . fst) formals
  concatM [ pure "(forall ("
...
valueToZ3 (VMap map) = freshEmitterVar "map" -- TODO: map same maps to same variable
...
symValueToZ3 (SymVar x t) = translateVar x
...
vcToZ3 vc = do
  ...
  vars <- mapM (translateVar . symVarName) $ collectSymVars vc
  forM_ vars $ \var ->
    emit $ z3Call "declare-const" [var, "Value"]
  forM_ (vc ^. assumptions) $ \asm -> do
    asmProp <- toZ3 asm
    emit $ z3Call "assert" [asmProp]
