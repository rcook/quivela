...
havocObj obj
  | obj ^. Q.objAdversary = return obj -- ^ this is a hack, since we currently don't support const annotations on global variables
  | otherwise = do
    newLocals <-
      Monad.mapM
...
checkWithZ3 vc = do
  ...
  ((), vcLines) <- fmap (Arrow.second L.nub) . runEmitter pctx $ toZ3_ vc
  sendToZ3 "(push)"
...
writeToZ3File dir vc = do
  ...
  (_, vcLines) <- fmap (Arrow.second L.nub) . runEmitter pctx $ toZ3 vc
  tempFile <-
